...
public class ListTableModel <T> implements TableModel {
    ...
    private ArrayList<ArrayList<T>> source;

    public ListTableModel(ArrayList<ArrayList<T>> source) {
    ...
    public String getColumnName(int columnIndex) {
        ...
        return (String) source.get(0).get(columnIndex);
    }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ArrayList<T> row = source.get(rowIndex);
        if(columnIndex >= row.size() || rowIndex == 0)
            return null;
        else
